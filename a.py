...
def connectHandler2(data):
    query = 'SELECT tb.action, tb.time, u.email from tableActions tb LEFT JOIN users u ON u.id=tb.userID ORDER BY tb.time DESC LIMIT 10'
    cursor = db_connection.cursor()
    cursor.execute(query)
...
def expertalSystem():
    ...
    db_connection.commit()
    dictionary = {}
    dataDb = tuple_to_dict(dataDb, dictionary)

    #hum, temp = Adafruit_DHT.read_retry(11, 3)
    temp = 21
    lights = dataDb['main_light']
    window1 = dataDb['window1']
    ...
    if motion_sensor_value==0 or motion_sensor_value==1:
        if motion_sensor_value==1:
            ...
            motion = False
    if window1 == True or window2 == True:
        windows = True
